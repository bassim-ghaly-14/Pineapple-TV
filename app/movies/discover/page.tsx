import { discoverMovies, getMovieGenres } from "@/lib/tmdb/endpoints";
...
export default async function DiscoverMoviesPage({ searchParams }: Props) {
  ...
  const [genres, data] = await Promise.all([getMovieGenres(), discoverMovies(filters)]);

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-text sm:text-3xl">Discover Movies</h1>
      <FilterBar
        genres={genres}
